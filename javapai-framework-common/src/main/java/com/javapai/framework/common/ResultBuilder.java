...
public final class ResultBuilder {
	 ...
	 * 通常情况下，我们不建议使用此方法来构造一个返回对象，因为自定义的code->message无法管理，容易造成code lost
	 * control。<br>
	 * 所以我们建议你使用{@link ResultBuilder#buildResult(Enums enums)}代替本方法；当然，你也可以使用{@link ResultBuilder#nomarResult}来明确返回一个正常对象或使用{@link ResultBuilder#errorResult}返回一个错误对象。<br>
	 * <br>
	 *
	 * @param code
	 *            自定义编码。<br>
	 ...
	 *
	 */
	public static <T> RstResult<T> buildErrorResult(String code, String message) {
		RstResult<T> entity = new RstResult<>();
		if ("".equals(code) || !"".equals(message)) {
			code = "42000001";
		}
	 ...
	 * 
	 */
	public static <T> RstResult<T> buildNormalResult(String code, String message) {
		RstResult<T> entity = new RstResult<>();
		if ("".equals(code) || !"".equals(message)) {
			code = "42000001";
		}
